...
public final class UpdateValue extends RiakCommand<UpdateValue.Response, Location>
{
    ...
    protected RiakFuture<Response, Location> executeAsync(final RiakCluster cluster)
    {
        ...
        RiakFutureListener<FetchValue.Response, Location> fetchListener = fetchListenerFuture ->
        {
            if (!fetchListenerFuture.isSuccess())
            {
                updateFuture.setException(fetchListenerFuture.cause());
                return;
            }
            ...
            try
            {
                fetchResponse = fetchListenerFuture.get();
                ...
                {
                    Response updateResponse = new Response.Builder().withLocation(fetchListenerFuture.getQueryInfo())
                                                                    .withUpdated(false)
                                                                    .build();
