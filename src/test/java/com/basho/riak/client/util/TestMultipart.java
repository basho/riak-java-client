...
public class TestMultipart {
    ...
    @Test public void parses_subpart_headers_and_body() { 
        ...
        assertEquals("a85hYGBgzGDKBVIsLOLmazKYEhnzWBkmzFt4hC8LAA==", parts.get(0).getHeaders().get(Constants.HDR_VCLOCK));
        assertEquals("/raw/test/key", parts.get(0).getHeaders().get(Constants.HDR_LOCATION));
    ...
    @Test public void handles_quoted_boundary() {
        Map<String, String> headers = new HashMap<String, String>();
        headers.put("content-type", "multipart/mixed; boundary=\"\\\\\\x\\\"\""); // boundary is the string: \x"
        String body =  
            "\n--\\x\"\n" +
            "\n" +
            "part\n" +
